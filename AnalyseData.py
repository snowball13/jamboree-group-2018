...
def get_prediction_data():
    ...
    ############# Format output
    indicesout = GPC[GPCindex, 0].astype(np.float64)
    GPCout = GPC[GPCindex, 1].astype(np.float64)
    GGDPout = GGDP[GGDPindex, 1].astype(np.float64)
    GGHFout = GGHF[GGHFindex, 1].astype(np.float64)
    GPGAout = GPGA[GPGAindex, 1].astype(np.float64)
    GASout = GAS[GASindex, 1].astype(np.float64)
    GMIPCout = GMIPC[GMIPCindex, 1].astype(np.float64)
    GSBout = GSB[GSBindex, 1].astype(np.float64)
    UHFout = UHF[UHFindex, 1].astype(np.float64)
    GNLout = GNL[GNLindex, 1].astype(np.float64)
